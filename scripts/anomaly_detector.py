...
def parse_opt():
    ...
    parser.add_argument('--train_method', default = 'DOMINANT',
                        help = 'Graph based Neural Network methods, DOMINANT or GCNEnDe')
    parser.add_argument('--sample_method', default = '3zones', help = 'Data sampling methods, 3zones or time_dominant')
    parser.add_argument('--dataset_path', default = 'sms-call-internet-mi-2013-11-01.txt',
                        help = 'dataset CDR choice (processed)')
    parser.add_argument('--save_model', default = True, help = 'save model')
    parser.add_argument('--zones', default = 'Low', help = 'zones for detect, High, Middle, Low or Shuffle')
    parser.add_argument('--load_model', nargs = '?', default = None,
                        help = 'Load and Use previously trained model, e.g. model_001.pt')
    parser.add_argument('--visualization', default = False, help = 'show graph img')
    parser.add_argument('--gen_outliers_method', default = 'candidates_replace',
                        help = 'the method to generate outliers, candidates_replace or manually')
...
def train(ids,
    ...
    for id in ids:
        ...
        x = torch.tensor(
            zone_data[['time_in_minutes', 'cell_id',
...
def main():
    ...
    for datasets in datasets_list:
        ...
        if opt.train_method in ['DOMINANT', 'GCNEnDe']:
            # -----------sampling : 3zones, 24 time points per day------------------------------------------------------
            # calculate the average activities of each zone, and rank the 100 zones,
            # divide into 3 classes: High active, Low active, inactive
            if opt.sample_method == '3zones':
                ...
                activity_ids = {
                    'Low': Low_activity_ids,
                    'Middle': Middle_activity_ids,
                    'High': High_activity_ids
                }
                ...
                if opt.zones in activity_ids:
                    train_args['ids'] = activity_ids[opt.zones]
                else:
                    train_args['ids'] = High_activity_ids + Middle_activity_ids + Low_activity_ids
                    random.shuffle(train_args['ids'])

                detector = train(**train_args)
            # -----------sampling : 3zones, keep all time points--------------------------------------------------------
            # -----------training : transformer-------------------------------------------------------------------------
            elif opt.sample_method == 'time_dominant':
                sampled_time = np.arange(30, 30 + 24 * 60, 60)
                core_df = core_df[core_df['time_in_minutes'].isin(sampled_time)]
